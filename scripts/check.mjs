...
assert.match(main, /function playShowdownAttackAttemptSounds/, "piece attack sounds should be reusable for hit and miss attempts");
assert.match(main, /if \(dx > ARENA\.attackRange\) \{[\s\S]*playShowdownAttackAttemptSounds\(attackerPiece, fighter, "light_attack"\)/, "basic attack misses should still play the attack sound");
assert.match(main, /playShowdownAttackAttemptSounds\(attackerPiece, fighter, critical \? "critical" : "light_attack"\)[\s\S]*dealCombatDamage/, "basic attack sounds should play before damage result checks");
assert.doesNotMatch(main, /attackerPiece\.type === "pawn" && dealt > 0[\s\S]*playPawnShowdownSound/, "pawn attack sounds should not depend on dealt damage");
assert.doesNotMatch(main, /attackerPiece\.type === "rook" && dealt > 0[\s\S]*playRookShowdownSound/, "rook attack sounds should not depend on dealt damage");
assert.doesNotMatch(main, /isBlackKnightSoundPiece\(attackerPiece\) && dealt > 0[\s\S]*playBlackKnightShowdownSound/, "black knight attack sounds should not depend on dealt damage");
...
assert.match(main, /function playShowdownAttackAttemptSounds[\s\S]*isBlackKnightSoundPiece\(piece\)[\s\S]*playBlackKnightShowdownSound\(fighter\.team, soundType\)[\s\S]*playBlackKnightVoiceSound\(fighter\.team, "light_attack"\)/, "black knight basic attacks should play Showdown and voice sounds like pawns and rooks");
assert.match(main, /function playCriticalHitReactionSound[\s\S]*isBlackKnightSoundPiece\(piece\)[\s\S]*playBlackKnightVoiceSound\(fighter\.team, "critical"\)/, "black knight should play a critical-hit voice reaction when hit by a critical attack");
